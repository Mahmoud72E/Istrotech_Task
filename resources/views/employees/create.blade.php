        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1 class="text-2xl font-semibold mb-4">{{ __("Create Employee") }}</h1>
                    <form method="POST" action="{{ route('employees.store') }}">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="birth_date" name="birth_date" type="date" class="mt-1 block w-full" :value="old('birth_date')" autofocus autocomplete="birth_date" />
                            ...
                            <div>
                                ...
                                <select name="user_id" id="user_id" class='border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm'>
                                    ...
                                    @foreach ($managers as $manager)
                                        <option value="{{ $manager->id }}" @selected(old('user_id'))>{{ $manager->name }} - {{ $manager->email }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="flex items-center gap-4">
                            <button type="submit" class="create-button">{{ __('Add Employee') }}</button>
                        </div>
                    </form>
                </div>
            </div>
